...
class MonitoringCollector:
    ...
    def getconfig(self, filename, target_hint):
        ...
        for host in hosts:
            ...
            tmp = {}
            tmp.update({'metric': metric or 'cpu-stat'})
            tmp.update({'interval': host.get('interval') or 1})
            tmp.update({'priority': host.get('priority') or 0})
            tmp.update({'port': host.get('port') or '22'})
            tmp.update({'python': host.get('python') or '/usr/bin/python'})
